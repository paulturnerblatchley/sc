app.component(
	'dashboard',
	{
        ...
        controller: function($scope, $rootScope, auth, uiGridConstants, $sessionStorage, $state, proforma, partners, tasks, sections) {
            ...
            $scope.deletePartner = function(obj) {
            	var partner = {};
            	for(i in obj) {
            		if (i === "lender") {
            			partner.table = 'lenders';
            			partner.column = i;
            			partner.value = obj[i];
            		} else if (i === "entity") {
            			partner.table = 'entity_vesting';
            			partner.column = i;
            			partner.value = obj[i];
            		} else if (i === "supervisor") {
            			partner.table = 'supervisors';
            			partner.column = i;
            			partner.value = obj[i];
            		} else if (i === "manager") {
            			partner.table = 'asset_managers';
            			partner.column = i;
            			partner.value = obj[i];
            		} else if (i === "name") {
                        partner.table = 'sections';
                        partner.column = i;
                        partner.value = obj[i];
                    }  else if (i === "task_name") {
                        partner.table = 'tasks';
            	...
            	auth.post('deletePartner', {
            		partner: partner
            	}).then(function(results) {
            		var index = $scope[partner.table].indexOf(partner.value);
            		$scope[partner.table].splice(index,1);
            	})
            }
